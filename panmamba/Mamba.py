...
from panmamba.refine import Refine
from model.GNN1 import *
from model.PGCU import *
# from upsamplinglg import upsamplinglg
...
class PanMamba(nn.Module):
    def __init__(self,num_channels=None,base_filter=None,args=None):
        ...
        self.gnnmodel = adjGNN(4)
        self.ConvTrans1 = nn.Sequential(
            nn.ConvTranspose2d(in_channels=4, out_channels=4, kernel_size=(3, 3), stride=2,
    ...
    def forward(self,ms,pan):
        ...
        # ms_p = F.interpolate(ms,scale_factor=4)
        # ms_bic = ms_p + self.PGCU(pan, ms)
        ms = self.ConvTrans1(ms)
        #ms = F.interpolate(ms,scale_factor=4)
        ms_bic = ms + self.gnnmodel(ms, pan).to(device)
        #ms_bic = self.gnnmodel(ms, pan).to(device)
