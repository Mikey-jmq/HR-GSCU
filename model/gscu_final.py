...
class Encoder1(nn.Module):
    def __init__(self, channels=4, n_feats=48,n_classes=50):
        ...
        self.v_fusion = nn.Sequential(
            nn.Conv2d(channels , channels * 2, kernel_size=3, padding=1, groups=channels),
            nn.ReLU(),
        ...
        self.downsample_fusion_mid = nn.Sequential(
            nn.Conv2d(channels , n_feats, kernel_size=3, padding=1),
            nn.ReLU()
        )
        self.downsample_fusion_low = nn.Sequential(
            nn.Conv2d(channels, n_feats, kernel_size=3, stride=2, padding=1),
            nn.ReLU(),
            nn.MaxPool2d(2)
        )
        self.dilated = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(channels , n_feats // 4, kernel_size=3, padding=ks, dilation=ks),
                nn.ReLU()
            ) for ks in [1, 2, 3]
        ])
        self.global_pool = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(channels, n_feats // 4, kernel_size=1),
            nn.ReLU()
        )
        self.residual = nn.Sequential(
            nn.Conv2d(channels , n_feats, kernel_size=1),
            nn.ReLU()
        )
    ...
    def forward(self, ms, pan):
        input = nn.functional.interpolate(ms, scale_factor=4, mode='bilinear')  # (B, 4, 128, 128)
...
class Encoder(nn.Module):
    ...
    def forward(self, ms, pan):
        target_h, target_w = pan.shape[-2:]

        up_ms = nn.functional.interpolate(ms, size=(target_h, target_w), mode='bilinear')
        pan_feat = self.pan_conv(pan)
        input = torch.cat([up_ms, pan_feat], dim=1)

        # V 分支
        V = self.v_fusion(input)
        ...
        mid_feat = self.downsample_fusion_mid(down_input)
        low_feat = self.downsample_fusion_low(down_input)
        up_mid = F.interpolate(mid_feat, size=(target_h, target_w), mode='bilinear')
        up_low = F.interpolate(low_feat, size=(target_h, target_w), mode='bilinear')

        feats = [branch(input) for branch in self.dilated]
        gp = self.global_pool(input)
        ...
        # Logits
        coords = make_coord(pan.shape[-2:], flatten=False).to(pan.device)
...
class GaussianSplatter(nn.Module):
    ...
    def query_rgb(self):
        ...
        B, _, curr_H, curr_W = self.feat.shape
        ...
        final_image = F.interpolate(final_image, size=(curr_H, curr_W), mode='bicubic', align_corners=False)
...
class GaussianSplatter_noCnoP(nn.Module):
    """A module that applies 2D Gaussian splatting to input features."""
    ...
    def __init__(self, kernel_size=5, unfold_row=8, unfold_column=8, c1=6, channels=4, n_feats=48,n_classes=50):
        ...
        Args:
            ...
            n_feats: The channels of features from Encoder.
        """
        super(GaussianSplatter_noCnoP, self).__init__()
        self.encoder = Encoder2(channels=channels, n_feats=n_feats,n_classes=n_classes)
        self.feat, self.logits = None, None  # 后续会有编码器生成这个
        # Key parameter in 2D Gaussian Splatter参数
        self.kernel_size = kernel_size
        self.row = unfold_row
    ...
    def gen_feat(self, ms, pan):
        """Generate feature and logits by encoder."""
        self.ms = ms
        self.feat, self.logits = self.encoder.forward(ms, pan)
        return self.feat, self.logits
    ...
    def query_rgb(self):
        ...
        #unfold_V1_weights = torch.softmax(unfold_V1_reshaped, dim=1)  # (B, 8, 64)
        kernel_channel = kernel_normalized.unsqueeze(0).unsqueeze(2).repeat(batch_size, 1, channel, 1,1)  # (B, 64, 8, kernel_size, kernel_size)
        kernel_color = kernel_channel.view(batch_size * num_LR_points, channel, self.kernel_size,self.kernel_size)  # (B*64, 8, kernel_size, kernel_size)
...
class GaussianSplatter_noC(nn.Module):
    """A module that applies 2D Gaussian splatting to input features."""
    ...
    def __init__(self, kernel_size=5, unfold_row=8, unfold_column=8, c1=6, channels=4, n_feats=48,n_classes=50):
        ...
        Args:
            ...
            n_feats: The channels of features from Encoder.
        """
        super(GaussianSplatter_noC, self).__init__()
        self.encoder = Encoder3(channels=channels, n_feats=n_feats,n_classes=n_classes)
        self.feat, self.logits = None, None  # 后续会有编码器生成这个
        # Key parameter in 2D Gaussian Splatter参数
        self.kernel_size = kernel_size
        self.row = unfold_row
