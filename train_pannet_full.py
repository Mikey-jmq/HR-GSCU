...
GNNPanNet = PanNet(4).to(device)
# GNNPanNet.load_state_dict(torch.load('best_results/PanNet/GF2/best_model.pth'))
...
for epoch in tqdm(range(epoches)):
    ...
    for label, pan, lrms, up_ms, hpan, hlrms in tqdm(train_loader, desc=f"Epoch {epoch} Training"):
        ...
        out = GNNPanNet.forward(pan, lrms, hlrms, hpan)
    ...
    if epoch % 1 == 0:
        ...
        if current_avg_qnr > best_qnr:
            ...
            # 只保存全尺度下 QNR 最高的那个权重
            best_filename = 'new_test/full/best_pannet2_full_gf2.pth'
