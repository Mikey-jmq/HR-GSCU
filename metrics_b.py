...
def D_lambda1(img_fake, img_lm, block_size=32, p=1):
    """Spectral distortion
...
def D_s1(img_fake, img_lm, pan, satellite='QuickBird', scale=4, block_size=32, q=1):
    """Spatial distortion
    img_fake, generated HRMS
    img_lm, LRMS
    pan, HRPan"""
...
def qnr(img_fake, img_lm, pan, satellite='QuickBird', scale=4, block_size=32, p=1, q=1, alpha=1, beta=1):
    ...
    D_lambda_idx = D_lambda1(img_fake, img_lm, block_size, p)
    D_s_idx = D_s1(img_fake, img_lm, pan, satellite, scale, block_size, q)
...
def no_ref_evaluate(pred, pan, hs):
    ...
    c_D_lambda = D_lambda1(pred, hs)
    c_D_s = D_s1(pred, hs, pan)
    c_qnr = qnr(pred, hs, pan)
    return [c_D_lambda, c_D_s, c_qnr]


def get_metrics_full(img_fake, img_lm, pan, satellite='QuickBird', scale=4, block_size=32, p=1, q=1, alpha=1, beta=1):
    img_lm = img_lm.squeeze(0).permute(1, 2, 0).cpu().numpy()
    img_fake = img_fake.squeeze(0).permute(1, 2, 0).detach().cpu().numpy()
    pan = pan.squeeze(0).permute(1, 2, 0).cpu().numpy()
    m1 = D_lambda1(img_fake, img_lm, block_size, p)
    m2 = D_s1(img_fake, img_lm, pan, satellite, scale, block_size, q)
    m3 = (1 - m1) ** alpha * (1 - m2) ** beta
