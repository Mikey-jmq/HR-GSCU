...
def sam(img1, img2):
    ...
    img1_ = img1.detach().cpu().numpy().astype(np.float64)
    img2_ = img2.detach().cpu().numpy().astype(np.float64)
...
def scc(img1, img2):
    ...
    if not  img1.shape == img2.shape:
        raise ValueError('Input images must have the same dimensions.')
    img1_ = img1.cpu().detach().numpy().astype(np.float64)
    img2_ = img2.cpu().detach().numpy().astype(np.float64)
    if img1_.ndim == 2:
...
def ergas(img_fake, img_real, scale=4):
    ...
    if not img_fake.shape == img_real.shape:
        raise ValueError('Input images must have the same dimensions.')
    img_fake_ = img_fake.cpu().detach().numpy().astype(np.float64)
    img_real_ = img_real.cpu().detach().numpy().astype(np.float64)
    if img_fake_.ndim == 2:
...
def mtf_resize(img, satellite='QuickBird', scale=4):
    ...
    # img_ = img_.type(torch.float32).cpu().detach().numpy()
    if img_.ndim == 2:  # Pan
        H, W = img_.shape
